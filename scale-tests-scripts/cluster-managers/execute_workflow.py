...
if __name__ == '__main__':
    ...
    for i in range(max_threads):
        if count < dep_for_each:
            create_threads(threads, client1, endpoint_dict, lock, workflow)
        elif count < 2*dep_for_each:
            create_threads(threads, client2, endpoint_dict, lock, workflow)
        else:
            create_threads(threads, client3, endpoint_dict, lock, workflow)
        count = count + 1
